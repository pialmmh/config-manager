...
//    private Set<DialplanPrefix> dialplanPrefixes;
    ...
    // Utility method to get prefixes directly
    @JsonIgnore
    public Set<DialplanPrefix> getDialplanPrefixes(DialplanPrefixRepository dialplanPrefixRepository) {
        return dialplanMappings.stream()
                .map(mapping -> dialplanPrefixRepository.findById(mapping.getIdDialplanPrefix())
                        .orElseThrow(() -> new RuntimeException("DialplanPrefix not found for ID: " + mapping.getIdDialplanPrefix())))
